...

{
    public class ColliderCapsule : MonoBehaviour
    {
        ...
        //     return -1;
        // }
        public int CheckCapsuleCollision(ISphere ene,Vector3 localDiffPlace, Vector3 localPoint, float maxLen, float maxSeLen){
            ...
            if (maxLen + ene.Radius < absN)
            {
                return -1;
            }
            else
            {
                if (maxLen - maxSeLen <= absN && absN <= maxLen + ene.Radius)
                {
                    ...
                    //            ene.transform.localScale.x);
                    if (circleDistance.magnitude < ene.Radius + maxSeLen)
                    {
                ...
                if (absN < maxLen - maxSeLen)
                {
                    ...
                    if (lineDistance.magnitude < ene.Radius + maxSeLen)
                    {
                        Debug.Log("カプセル判定");
                        return 1;
                    }
